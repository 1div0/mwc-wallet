...
pub trait OwnerRpcV2: Sync + Send {
	...
	```
	# grin_wallet_api::doctest_helper_json_rpc_owner_assert_response_v2!(
	# r#"
	{
		"jsonrpc": "2.0",
		"method": "accounts",
...
pub fn run_doctest_owner_v2(
	...
	let res = if use_token {
		panic!("V2 API never expect the token. Keeping it as it is");
	} else {
...
#[doc(hidden)]
#[macro_export]
macro_rules! doctest_helper_json_rpc_owner_assert_response_v2 {
	($request:expr, $expected_response:expr, $use_token:expr, $blocks_to_mine:expr, $perform_tx:expr, $lock_tx:expr, $finalize_tx:expr, $payment_proof:expr) => {
		...
		#[cfg(not(target_os = "windows"))]
			{
			use grin_wallet_api::run_doctest_owner_v2;
			use serde_json;
			...
			let response = run_doctest_owner_v2(
				request_val,
				dir,
				$use_token,
				$blocks_to_mine,
